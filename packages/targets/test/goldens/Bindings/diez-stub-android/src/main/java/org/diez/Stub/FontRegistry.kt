...
internal fun FontRegistry.Companion.registerSystemFonts() {
...
private fun hotLoadFont(file: File) {
...
private fun loadFont(file: File) {
    val resourceFile = Environment.resources.openRawResource(file.resourceId)
    val ttf = TTFFile.open(resourceFile)
    foreignFontCache.set(ttf.postScriptName, Environment.resources.getFont(file.resourceId))
}

fun FontRegistry.registerFonts() {
    files.forEach {
